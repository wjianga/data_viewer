...
cnv = read.csv("./files/CNV_ExomeDepth.csv")

plot_cnv <- function(start_pos, end_pos, chromosome) {
  subset_cnv <- cnv %>% 
    filter(chromosome == chromosome) %>% 
    filter(start >= start_pos & end <= end_pos)
  
  ggplot(subset_cnv) +
    geom_point(aes(x = start, y = reads.ratio, color = Hugo_Symbol), size = 4) +
    theme_bw() +
    labs(color = "Common CNV")
}

plot_cnv(0, 19732460, 1)
